...
function showHelp() {
  ...
  html+='Click the <i class="bx bx-file"></i> button to bring up the model list. From there you can load files into the webview by clicking on one of the models. Right-Clicking on a model brings up a menu that allows you to add the file to the existing content of the webviewer (useful for example if you want to load multiple federates BIM models) or to delete the file from your account.<br><br>';
  ...
  html+='To upload assemblies consisting of more than one files you have two options:<br>';
  html+='<ul><li>Click the "Upload as Assembly" checkbox and then drag & drop all files the assembly consist of into the upload area (you can also click on the upload area to select the files via the filemanager). Then select the top level assembly before clicking the upload button.</li>';
  html+='<li>Create a zip file of your assembly and drag it into the upload area (leave the "Upload as Assembly checkbox unchecked"). Instead of uploading the file immediatly you will be prompted to select the top level assembly first.</li></ul><br>';
  html+='Currently your account is ephemeral, meaning it is tied to your browser and any files you uploaded will only be available to you on the same browser if you reload the page. Any accounts that have not been used in 24 hours will be purged, including any uploaded files. You can also manually force a purge by selection "Reset Demo Project" from the top-right menu';
